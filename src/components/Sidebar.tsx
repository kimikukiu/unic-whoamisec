...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, target = "NONE", isUp = true, isAttacking = false }) => {
  const menuItems = [
    ...
    { id: AppTab.AI_CONFIG, label: '⚡ AI SETUP', icon: 'fa-cog' },
  ];

  const externalTools: { label: string; icon: string; url?: string; tab?: AppTab; color: string }[] = [
    { label: 'GPT CHAT', icon: 'fa-robot', tab: AppTab.GPT_TOOL, color: 'text-[#00ffc3]' },
    { label: 'IDE', icon: 'fa-laptop-code', tab: AppTab.IDE_TOOL, color: 'text-[#ffaa00]' },
    { label: 'SOLANA', icon: 'fa-link', tab: AppTab.SOLANA_TOOL, color: 'text-[#aa00ff]' },
    { label: 'DEPLOYER', icon: 'fa-rocket', tab: AppTab.DEPLOYER_TOOL, color: 'text-[#ff5e00]' },
    { label: 'QUANTUM', icon: 'fa-atom', tab: AppTab.QUANTUM_TOOL, color: 'text-[#00ffff]' },
    { label: 'SCANNER', icon: 'fa-search', tab: AppTab.SCANNER_TOOL, color: 'text-[#ff00ff]' },
    { label: 'S3 BUCKETS', icon: 'fa-bucket', tab: AppTab.S3_TOOL, color: 'text-[#f59e0b]' },
    { label: 'BLACKHAT', icon: 'fa-user-ninja', tab: AppTab.BLACKHAT_TOOL, color: 'text-[#ef4444]' },
    { label: 'BURPSUITE', icon: 'fa-spider', tab: AppTab.BURPSUITE_TOOL, color: 'text-[#ff6600]' },
    { label: 'OWASP ZAP', icon: 'fa-bug', tab: AppTab.OWASP_TOOL, color: 'text-[#3b82f6]' },
