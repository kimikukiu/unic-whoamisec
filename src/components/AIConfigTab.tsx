...
const AIConfigTab: React.FC = () => {
  ...
  return (
    <div className="p-4 space-y-4 bg-[#0a0a0a] text-white min-h-screen">
      {/* Connection Status */}
      <div className="bg-black/40 border border-emerald-500/20 rounded-lg p-4">
        ...
        <div className="flex items-center gap-3 text-xs">
          <i className={`fas ${providers.find(p => p.value === config.provider)?.icon} ${providers.find(p => p.value === config.provider)?.color}`}></i>
