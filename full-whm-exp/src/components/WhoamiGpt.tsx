...
import { whoamisecGptChat, localIntelligence, getAvailableModels, switchAIModel } from '../services/geminiService';
        ...
        <div className="flex items-center justify-between p-1.5 bg-gradient-to-r from-[#dc2626] to-[#991b1b]">
          ...
          <div className="flex items-center gap-2">
            <select 
              value={currentModel} 
              onChange={(e) => {
                switchAIModel(e.target.value);
                setCurrentModel(e.target.value);
                addLog(`SWARM: Switched to model ${e.target.value}`, "info");
              }}
              className="bg-black border border-[#dc2626] text-[#dc2626] rounded px-2 py-0.5 text-[7px] outline-none"
            >
