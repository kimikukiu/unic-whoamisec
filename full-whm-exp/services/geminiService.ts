...
const taskQueue = new AITaskQueue(HF_TOKEN);

export const switchAIModel = (model: string) => {
  taskQueue.setModel(model);
};

export const getAvailableModels = () => {
  return taskQueue.getAvailableModels();
};

if (HF_TOKEN) {
  console.log("WHOAMISEC_CORE: HuggingFace token detected. Status: ACTIVE (Llama-3.3-70B).");
} else {
  console.warn("WHOAMISEC_CORE: No HuggingFace token found. Set HF_API_TOKEN env var.");
}
